...
/*   Updated: 2024/07/01 16:29:48 by fbelotti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	handle_mutex(&(data)->philo->first_fork, LOCK);
	print_status(data, "has taken a fork");
	handle_mutex(&(data)->philo->second_fork, LOCK);
...
void	philo_is_eating(t_data *data)
{
	...
	ft_usleep(data->time->time_to_eat);
...
{
	handle_mutex(&(data)->philo->first_fork, UNLOCK);
	handle_mutex(&(data)->philo->second_fork, UNLOCK);
}

void	philo_is_sleeping(t_data *data)
{
	print_status(data, "is sleeping");
	ft_usleep(data->time->time_to_sleep);
}
